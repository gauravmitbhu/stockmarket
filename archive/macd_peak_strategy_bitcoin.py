...
def identify_macd_peaks(data):
    """
    Identify MACD histogram peaks.
    Green peaks: Histogram is positive and is reversing from negative or higher than the previous bar.
    Red peaks: Histogram is negative and is reversing from positive or lower than the previous bar.
    """
    data['Green_Peak'] = False
    data['Red_Peak'] = False

    # for i in range(1, len(data)):
    #     if data['MACD_Histogram'].iloc[i] > 0 and (data['MACD_Histogram'].iloc[i] > data['MACD_Histogram'].iloc[i-1] or data['MACD_Histogram'].iloc[i-1] < 0):
    #         data['Green_Peak'].iloc[i] = True
    #     elif data['MACD_Histogram'].iloc[i] < 0 and (data['MACD_Histogram'].iloc[i] < data['MACD_Histogram'].iloc[i-1] or data['MACD_Histogram'].iloc[i-1] > 0):
    #         data['Red_Peak'].iloc[i] = True

    for i in range(1, len(data)):
        if data['MACD_Histogram'].iloc[i] > 0 and data['MACD_Histogram'].iloc[i] > data['MACD_Histogram'].iloc[i-1] and data['MACD_Histogram'].iloc[i] > data['MACD_Histogram'].iloc[i+1]:
            data['Green_Peak'].iloc[i] = True
        elif data['MACD_Histogram'].iloc[i] < 0 and data['MACD_Histogram'].iloc[i] < data['MACD_Histogram'].iloc[i-1] and data['MACD_Histogram'].iloc[i] < data['MACD_Histogram'].iloc[i+1]:
            data['Red_Peak'].iloc[i] = True
