...
excel_file_path = 'AAPL_data_5yr.xlsx'
...
def calculate_holding_value(data, start_date, end_date, initial_investment):
    """Calculate the value of holding an asset from start_date to end_date, using the nearest available dates if specific dates are not present."""

    # Find the nearest start date
    nearest_start_date = data.index[abs(data.index - start_date).argmin()]
    # Find the nearest end date
    nearest_end_date = data.index[abs(data.index - end_date).argmin()]

    # Get the closing prices for the nearest dates
    start_price = data.loc[nearest_start_date, 'Close']
    end_price = data.loc[nearest_end_date, 'Close']
...
def trading_strategy(data, initial_capital):
    ...
    for i in range(1, len(data)):
        portfolio['total'][i] = portfolio['cash'][i - 1] + portfolio['holdings'][i - 1] * data['Close'][i - 1]
